...
\begin{block}{Zerlegungsalgorithmus(zu 3NF)}
  \begin{enumerate}
    \item Kanonische Überdeckung F$_c$ von F berechnen
    \item Relation für jede FD in F$_c$:
    \begin{itemize}
      \item X $\rightarrow$ Y wird zu: R$_1$(X,Y).
      \item X $\rightarrow$ Y,Z wird zu: R$_2$(X,Y,Z).
      \item X,Y $\rightarrow$ Z wird zu: R$_3$(X,Y,Z).
    \end{itemize}
    \item Sicherstellen, dass es eine Relation mit einem Schlüssel für R gibt(Ansonsten Schlüssel als Relation R$_{n+1}$ hinzufügen)
    \item Doppelte Schemata entfernen
  \end{enumerate}
\end{block}
